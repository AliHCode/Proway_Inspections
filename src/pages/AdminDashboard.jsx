...
export default function AdminDashboard() {
    ...
    useEffect(() => { if (activeTab === 'support') fetchSupportTickets(); }, [activeTab, fetchSupportTickets]);
            ...
            <main className="admin-page">
                ...
                <div className="admin-nav-bar" style={{ marginTop: '0.5rem', marginBottom: '1.5rem' }}>
                    <div className="admin-tabs">
                        ...
                        <button className={`admin-tab-btn ${activeTab === 'support' ? 'active' : ''}`} onClick={() => setActiveTab('support')} style={{ position: 'relative' }}>
                    ...
                    <div className="admin-nav-actions">
                        ...
                        {activeTab === 'support' && (
                            <div className="users-stat-pills">
                                {supportTickets.filter(t => t.status === 'open').length > 0 && (
                                    <span className="ustat-pill ustat-warning">🎫 {supportTickets.filter(t => t.status === 'open').length} Open</span>
                                )}
                            </div>
                        )}
                        <button className="btn btn-ghost btn-sm btn-refresh-global" 
                            onClick={() => { activeTab === 'users' ? fetchUsers() : activeTab === 'support' ? fetchSupportTickets() : fetchProjects(); }} 
                            disabled={loading || supportLoading}
                            title="Refresh Data"
                        >
                            <RefreshCw size={16} className={(loading || supportLoading) ? 'spinner' : ''} />
                        </button>
                    </div>
                </div>
                ...
                {activeTab === 'support' && (
                    <div className="admin-section">
                        ...
                        {supportLoading ? (
                            ...
                            const STATUS_CFG = { open: { color: '#3b82f6', bg: '#eff6ff' }, in_progress: { color: '#d97706', bg: '#fffbeb' }, resolved: { color: '#059669', bg: '#ecfdf5' }, closed: { color: '#64748b', bg: '#f1f5f9' } };
                            ...
                            async function handleReply(ticketId) {
                                ...
                                const { error } = await supabase
                                    ...
                                    .eq('id', ticketId);
                                if (!error) {
                                    showMsg('Reply sent & ticket resolved');
